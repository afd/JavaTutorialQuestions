...
**Step 1.** Create an interface, `StringStackIterator`, that offers the following methods:
...
**Step 2.** Add a new method to the `StringStack` interface:
...
* Create a fresh class for each of `StringStackArrayIterator` and `StringStack``ListIterator`.
...
* Use *inner classes*.  Again, let us discuss the ``array'' case; the ``list'' case is similar.
...
existence.  (Recall that standard classes cannot be declared private -- this would make such a class
...
**Advanced:** *anonymous* inner classes.  Some software engineers argue that it is
