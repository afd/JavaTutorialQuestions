...
Your task is to extend `GraphNode` with \emph{cloning} facilities.  Remember that
...
\vspace{1mm}\noindent\textbf{Step 1.} \emph{Warm up.}  To understand the `Cloneable`
...
throw a `CloneNotSupportedException`, which is a \emph{caught} exception.  Add a `throws` clause
...
When you run `main`, you should find that such an exception \emph{is} thrown.  This is because `GraphNode`
...
type of your overriding `clone()` method should be as \emph{specific} as possible, and if possible your method should
...
object, and using it as a \emph{stack}.  Push `this` on to the stack.  Then while the stack is not empty, pop an element, `old` say, from the stack.
...
To do this, you should iterate through the \emph{key set} of `oldToNew`.  For each old node that is a key, consider all of its successors.
