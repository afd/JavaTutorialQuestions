...
Each `TreeNode` can store an arbitrary object, which is called a \emph{key}.
...
each \emph{leaf} node of the tree will contain an empty `children` array.
...
Can you manage to do this by implementing `toString()` in a \emph{single} place?  See the end of the sheet
for a hint.
