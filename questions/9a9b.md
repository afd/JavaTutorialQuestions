...
**Step 2.** Next, Create an *interface* called `Tune`.  Implementations of
...
**Step 3.** Implement `Tune` with an *abstract* class, `AbstractTune`.
...
**Step 4.** Extend `AbstractTune` with a concrete class, `PhysicalTune`.
...
\vspace{1mm}
**Step 5.** Now your task is to implement *transposition* of tunes; that is, raising or lowering
...
\vspace{1mm}
**Step 6.** Now you can implement the `transpose` method in `AbstractTune`.  This method
should return a new `TransposedTune`.

\vspace{1mm}
**Step 7.** Finally, write a short demo program to illustrate that this is working.
