...
\begin{itemize}
\item *note name*, which is one of C, C\#, D, D\#, E, F, F\#, G, G\#, A, A\# or B

\item *note octave*, which is an integer determining the octave of the note

\item *note value*, which can be one of whole, half, quarter, eighth or sixteenth
\end{itemize}
...
\vspace{1mm}\noindent**Step 2.** Next, Create an *interface* called `Tune`.  Implementations of
...
\vspace{1mm}\noindent**Step 3.** Implement `Tune` with an *abstract* class, `AbstractTune`.
...
\vspace{1mm}\noindent**Step 4.** Extend `AbstractTune` with a concrete class, `PhysicalTune`.
...
\vspace{1mm}
\noindent**Step 5.** Now your task is to implement *transposition* of tunes; that is, raising or lowering
...
tune element to the *original* tune.  (See hint at the end of the sheet.)
...
\vspace{1mm}
\noindent**Step 6.** Now you can implement the `transpose` method in `AbstractTune`.  This method
should return a new `TransposedTune`.

\vspace{1mm}
\noindent**Step 7.** Finally, write a short demo program to illustrate that this is working.
...
\vspace{1mm}\noindent**Step 2.**  This should do the trick:

\begin{lstlisting}
interface Tune extends Iterable<TuneElement> { ... }
\end{lstlisting}

\vspace{1mm}\noindent**Step 3.**  Because `AbstractTune` implements `Tune`, which extends `Iterable<TuneElement>`,
...
\vspace{1mm}\noindent**Step 4.**  You can implement `addTuneElement` by simply calling `add` on the `List`
...
\vspace{1mm}\noindent**Step 5.** To implement `addTuneElement`, you need to be able to translate a `Note` into an equivalent
